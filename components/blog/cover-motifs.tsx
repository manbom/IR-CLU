...
export type Shape = {
  ...
  // Explicit rotation metadata (in addition to the CSS `transform` already baked
  // into `style` above) — the DOM/SVG cover renderer needs a real rotation-center
  // point, which SVG's `rotate(deg, cx, cy)` attribute requires explicitly; CSS
  // `transform-origin` percentages (used by the div-based Satori renderer below)
  // don't carry over to SVG semantics, so this can't be derived from `style` alone.
  rotate?: number;
...
function pill(key: string, x: number, y: number, w: number, h: number, color: string, rotate = 0): Shape {
  return {
    ...
    ...(rotate ? { rotate, rotateOrigin: { x, y } } : {}),
  };
}
...
export function getMotifShapes(name: MotifName, colors: [string, string] = [CYAN, VIOLET]): Shape[] {
  const [a, b] = colors;
...
// Kept for opengraph-image.tsx (Satori/ImageResponse), which renders these as
// absolutely-positioned divs — unchanged behavior from before this file was split.
export function renderMotif(name: MotifName, colors: [string, string] = [CYAN, VIOLET]) {
  return renderShapes(getMotifShapes(name, colors));
}
